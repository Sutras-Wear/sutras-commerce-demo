...
(() => {
  ...
  function validConfig() {
    try {
      ...
      if(u.protocol!=='https:' || !/^[a-z0-9-]+\.supabase\.co$/.test(u.hostname) || u.username || u.password || u.search || u.hash || u.pathname!=='/' || u.port) return false;
  ...
  function validate(row) {
    ...
    if(typeof row.name!=='string' || !row.name.trim() || row.name.length>180) throw new Error('Invalid product data');
    for(const k of ['available_quantity'])
    ...
    if(row.currency!=='ZMW' || !Array.isArray(row.sizes) || row.sizes.some(s=>!allowedSizes.includes(s)) || !['unconfirmed','available','unavailable'].includes(row.availability) || typeof row.is_low_stock!=='boolean') throw new Error('Invalid catalogue data');
    return true;
  }
  async function read() {
    ...
    try {
      ...
      const response=await fetch(`${config.url.replace(/\/$/,'')}/rest/v1/sutras_storefront_products?select=${fields}&order=id`,{headers,cache:'no-store',signal:controller.signal});
